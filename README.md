...
| Testing | Vitest v4 (475 tests, 24 files) |
| Runtime | Node 22 |
| **Ordering Service** | |
...
| `npm test` | Run 475 tests (vitest run) |
...
| `/signup` | Static | Self-registration form |
...
| `/admin/[venueId]/sales` | Dynamic | Sales dashboard: revenue chart (Shamsi dates), order history, summary stats, item aggregation, CSV export |
| `/admin/[venueId]/settings` | Dynamic | Venue settings, logo upload, member management, station schedules, photo display toggle |
| `/admin/[venueId]/orders` | Dynamic | Admin order management (table grid, order panel, tag filters, fuzzy search) |
| `/admin/[venueId]/billing` | Dynamic | Billing/subscription management (plan selection, payment, invoices) |
| `/admin/[venueId]/billing/pay` | Dynamic | Payment page with coupon input and invoice summary |
...
**Billing:** `GET /api/billing/plans`, `GET|POST /api/billing/subscription`, `GET /api/billing/invoices`, `POST /api/billing/payments`, `POST /api/billing/coupons/validate`, `GET /api/billing/callback`
...
**Items:** `GET|POST /api/venues/[id]/items`, `GET|PATCH|DELETE /api/venues/[id]/items/[id]`, `POST .../items/reorder`, `POST .../items/bulk-visibility`, `POST .../items/bulk-delete`, `POST .../items/import-csv`, `GET .../items/export-csv`, `POST|DELETE .../items/[id]/photo`, `GET|POST .../items/[id]/variants`, `GET|POST .../items/[id]/prices`, `GET|POST .../items/[id]/allergens`
...
**Sales:** `GET /api/venues/[venueId]/sales?range=daily|weekly|monthly|yearly|custom&start=&end=`, `GET .../sales/items`, `GET .../sales/export`

**Schedules:** `GET|POST /api/venues/[id]/schedules`

**Tables:** `GET|POST /api/venues/[id]/tables`, `PATCH|DELETE /api/venues/[id]/tables/[tableId]` (tags supported)
...
- CSRF: parallel CSRF protection via `mofe_csrf` cookie + `X-CSRF-Token` header on all mutations
...
### QR Menu Editor (merged into `/menu`)
...
475 tests across 24 files with real PostgreSQL test DB (472 pass, 3 pre-existing billing/zarinpal failures):

```
src/__tests__/api/auth.test.ts                    —  13 tests (signup, login, logout, password reset, rate limiting)
src/__tests__/api/billing.test.ts                 —  53 tests (plans, subscription, invoices, payments, coupons)
src/__tests__/api/categories.test.ts              —  12 tests (category CRUD, reorder, permissions)
src/__tests__/api/concurrent.test.ts               —   1 test (concurrent request handling)
src/__tests__/api/items.test.ts                   —  18 tests (item CRUD, reorder, bulk visibility, CSV import)
...
│                                                       sales aggregation)
src/__tests__/api/menu-slug.test.ts                —  10 tests (slug generation, uniqueness)
src/__tests__/api/photo.test.ts                   —   6 tests (photo upload, compression, deletion)
src/__tests__/api/sales.test.ts                    —  19 tests (sales auth, aggregation, isolation, response format)
src/__tests__/api/sales-items.test.ts              —  12 tests (item-level aggregation, category/hourly breakdown)
src/__tests__/api/sales-export.test.ts             —  12 tests (CSV export, formula injection protection)
...
src/__tests__/lib/csrf.test.ts                    —   6 tests (token generation, validation, mismatch rejection)
src/__tests__/lib/offline-queue.test.ts            —  27 tests (queue add/remove/list/replay/tempOrderId replacement)
src/__tests__/lib/ordering-proxy.test.ts           —  12 tests (proxy forwarding, release-table, error handling)
src/__tests__/lib/public-menu/publication.test.ts  —  21 tests (snapshot building, publish/unpublish workflow)
src/__tests__/lib/public-menu/renderer.test.ts     —  70 tests (HTML structure, Persian formatting, escaping, edge cases)
src/__tests__/lib/rate-limit.test.ts               —   8 tests (rateLimit helper — DB-backed, windows, edge cases)
src/__tests__/lib/subscription.test.ts            —  37 tests (trial creation, plan changes, proration, renew, expire)
src/__tests__/lib/zarinpal.test.ts                —  13 tests (payment request, verification, sandbox mode)
src/__tests__/components/SalesClient.test.ts       —  13 tests (toPersianDate, formatCurrency helpers)
...
│   │                           #   VenueTable, Sale, SaleItem, AuditLog, PasswordResetToken,
│   │                           #   RateLimitEntry, Session, Plan, Subscription,
│   │                           #   Invoice, Coupon, orders (ignored), order_items (ignored)
...
├── src/
│   ├── __tests__/              # Vitest test suite (24 files, 475 tests)
│   │   ├── api/                # Integration tests (10 files — auth, billing, categories, concurrent, integration, items, menu-slug, photo, sales, sales-export, sales-items)
...
│   │   ├── admin/
│   │   │   └── [venueId]/      # Admin pages (7 sections)
│   │   │       ├── menu/       # Menu management + QR/publish editor (merged)
│   │   │       ├── publications/ # Publication history
│   │   │       ├── settings/   # Venue settings + members
│   │   │       ├── sales/      # Sales dashboard with revenue chart (Shamsi dates)
│   │   │       ├── orders/     # Admin order management (table grid + order panel)
│   │   │       └── billing/    # Billing/subscription management + payment page
...
│   ├── components/orders/      # 5 ordering components
│   │   ├── TableGrid.tsx       # Interactive table grid (with tag pills)
│   │   ├── OrderPanel.tsx      # Order detail + item actions (qty +/-, notes)
│   │   ├── MenuItemBrowser.tsx # Modal item browser for adding to order
│   │   ├── TableTagFilter.tsx  # Filter tables by selected tags
│   │   └── TagInput.tsx        # Tag entry with suggestions
...
│   │   ├── compress-image.ts   # Image compression via Sharp (WebP, ≤50KB)
...
│   │   ├── menu-cache.ts       # In-memory cache for published menus
│   │   ├── offline-queue.ts    # Client-side offline queue (localStorage)
...
│   │   ├── subscription.ts     # Billing helpers (plans, limits, change plan, renew)
│   │   ├── tables.ts           # Table status management helpers
│   │   ├── tag-colors.ts       # 10 rotating color schemes for table tags
│   │   ├── zarinpal.ts         # Zarinpal payment gateway (sandbox/mock)
│   │   ├── useFuzzySearch.ts   # Fuse.js-based fuzzy search hook
│   │   ├── useOfflineSync.ts   # Offline queue replay hook
